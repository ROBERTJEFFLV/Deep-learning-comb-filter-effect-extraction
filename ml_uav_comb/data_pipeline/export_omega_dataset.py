...
def _dataset_build_spec(cfg: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "dataset": {
            ...
            "normalization_split": cfg["dataset"].get("normalization_split", "auto"),
        },
        "model": {
...
def _resolve_normalization_split(cfg: Dict[str, Any], index_manifest: Dict[str, Any]) -> str:
    requested = str(cfg.get("dataset", {}).get("normalization_split", "auto")).strip().lower()
    split_counts = {
        str(split_name): int(split_info.get("num_windows", 0))
        for split_name, split_info in index_manifest.get("splits", {}).items()
    }
    total_windows = int(sum(split_counts.values()))
    if total_windows <= 0:
        raise ValueError("cannot compute omega normalization without any indexed windows")

    if requested in {"", "auto"}:
        if bool(cfg.get("dataset", {}).get("dynamic_epoch_split", False)):
            return "all"
    ...
    if requested not in split_counts:
        available = ", ".join(sorted(split_counts)) or "<none>"
        raise ValueError(
            f"dataset.normalization_split={requested!r} is not a valid split; available splits: {available}, all"
        )
    if int(split_counts[requested]) <= 0:
        non_empty = [name for name, count in split_counts.items() if int(count) > 0]
        raise ValueError(
            f"dataset.normalization_split={requested!r} has zero windows; non-empty splits: {non_empty}"
        )
    return requested
...
def build_omega_dataset(cfg: Dict[str, Any], build_jobs: Optional[int] = None) -> Dict[str, Any]:
    ...
    normalization_path = Path(cfg["dataset"]["normalization_path"])
    normalization_split = _resolve_normalization_split(cfg, index_manifest)
    ...
    meta = {
        ...
        "normalization_split": str(normalization_split),
