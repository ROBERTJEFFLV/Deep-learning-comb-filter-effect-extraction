...
def _dynamic_epoch_recording_codes(
    ...
    all_codes = np.asarray(sorted({int(code) for code in np.asarray(pooled_dataset.recording_code).tolist()}), dtype=np.int64)

    # Respect fixed_test_recording_codes: keep test recordings fixed,
    # only shuffle remaining recordings between train and val.
    fixed_test_codes_cfg = cfg.get("dataset", {}).get("fixed_test_recording_codes", None)
    if fixed_test_codes_cfg is not None:
        fixed_test_set = set(int(c) for c in fixed_test_codes_cfg)
        pool_codes = np.asarray([c for c in all_codes if int(c) not in fixed_test_set], dtype=np.int64)
        test_codes = np.asarray([c for c in all_codes if int(c) in fixed_test_set], dtype=np.int64)
        rng = np.random.default_rng(int(cfg.get("experiment", {}).get("seed", 0)) + int(epoch))
        shuffled_pool = rng.permutation(pool_codes)
        # Split pool into train and val using first two elements of ratio
        tv_ratio = ratio[:2]
        tv_counts = _allocate_dynamic_split_counts(int(shuffled_pool.shape[0]), tv_ratio)
        code_map: Dict[str, np.ndarray] = {}
        code_map["train"] = np.asarray(shuffled_pool[:int(tv_counts[0])], dtype=np.int64)
        code_map["val"] = np.asarray(shuffled_pool[int(tv_counts[0]):], dtype=np.int64)
        code_map["test"] = test_codes
        return code_map

    # Fallback: shuffle all recordings across train/val/test
    rng = np.random.default_rng(int(cfg.get("experiment", {}).get("seed", 0)) + int(epoch))
    shuffled_codes = rng.permutation(all_codes)
